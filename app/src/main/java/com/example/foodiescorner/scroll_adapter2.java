...
public class scroll_adapter2 extends RecyclerView.Adapter<scroll_adapter2.ViewHolder> {

    ArrayList<scroll_model2> modelArrayList1;

    public scroll_adapter2(ArrayList<scroll_model2> modelArrayList1) {
        this.modelArrayList1 = modelArrayList1;
    }

    @NonNull
    @Override
    public scroll_adapter2.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.horizontal_scroll_item2,parent,false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull scroll_adapter2.ViewHolder holder, int position) {
        String resource=modelArrayList1.get(position).getProductImage();
        String rupees=modelArrayList1.get(position).getProductMoney();
        String product_title=modelArrayList1.get(position).getProductTitle();
